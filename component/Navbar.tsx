...
<div className="flex items-center justify-between w-full md:w-auto">
  ...
  className="relative md:hidden inline-flex items-center justify-center w-10 h-10 rounded-md border border-white/30 hover:border-white/60 transition"
  onClick={() => setIsOpen((v) => !v)}
>
  {/* Hamburger / Close Icon */}
  <span aria-hidden className="block w-5 h-[2px] bg-white relative">
    <span
      className={`absolute left-0 top-[-6px] w-5 h-[2px] bg-white transition-transform ${
  ...
  {/* Cart Count Badge → only show when toggle is closed */}
  {!isOpen && cartCount > 0 && (
...
{isOpen && (
  <div className="md:hidden w-full mt-4">
    ...
    <Link href="/booking" className="relative w-full" onClick={() => setIsOpen(false)}>
      <button className="mt-4 w-full px-6 py-2 rounded-lg border border-white/60 hover:border-white transition-colors duration-200 hover:bg-white/10 flex items-center justify-center gap-2 relative">
        <Car className="h-5 w-5" />
        Get a quote
        {cartCount > 0 && (
